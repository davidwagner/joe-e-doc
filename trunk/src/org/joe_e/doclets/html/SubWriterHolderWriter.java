...
public abstract class SubWriterHolderWriter extends HtmlDocletWriter {
    ...
    public void printSummaryLinkType(AbstractMemberWriter mw,
        ...
        MemberInfo m = configuration.consumer.getInfo(member);
    	if (m == null || !m.enabled) {
        	trBgcolorStyle("#F0F0F0", "");
            mw.printSummaryType(member);
            summaryRow(0);
            code();
        } else {
            trBgcolorStyle("#FFFFFFFF", "TableRowColor");
            mw.printSummaryType(member);
            summaryRow(0);
            code();
        }
    ...
    public void printSummaryLinkComment(AbstractMemberWriter mw,
        ...
        printNbsps();
        String className = member.containingClass().qualifiedName();
        if (!configuration.consumer.isDisabled(className)) {
            MemberInfo m = configuration.consumer.getInfo(member);
            if (m == null || !m.enabled) {
                print("<B>Suppressed.</B>");
                br();
                printNbsps();
            }
        }
