/*
 * @(#)ConstructorWriterImpl.java	1.41 03/12/19
 *
...
public class ConstructorWriterImpl extends AbstractExecutableMemberWriter
    ...
    public void writeComments(ConstructorDoc constructor) {
    	...
    	String className = constructor.containingClass().qualifiedName();
        MemberInfo m = writer.configuration.consumer.getInfo(constructor);
        if (m == null) {
            writer.dd();
            if (writer.configuration.consumer.isDisabled(className)) {
                ...
                String signature = name.substring(name.lastIndexOf(".") + 1)
                                   + constructor.flatSignature();
                System.err.println("WARNING: no policy for " + className + "." + signature);
