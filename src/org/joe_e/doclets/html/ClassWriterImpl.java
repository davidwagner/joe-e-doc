...
import org.joe_e.doclets.internal.toolkit.builders.*;
...
public class ClassWriterImpl extends SubWriterHolderWriter
    ...
    public void writeHeader(String header) {
        ...
        printHtmlHeader(clname,
            ...
            configuration.consumer.isDisabled(classDoc.qualifiedName()));
        navLinks(true);
        hr();
    ...
    public void writeClassDescription() {        
        if(!configuration.nocomment) {
        	String s = this.configuration.consumer.getClassDescription(classDoc.qualifiedName());
        	boolean disabled = this.configuration.consumer.isDisabled(classDoc.qualifiedName());
        	if (disabled) {
        		print ("<font color=\"red\"><B>Disabled: </B> ");
        		s = this.configuration.consumer.getDisabledComment(classDoc.qualifiedName());
        		if (s != null && !s.equals("")) {
        			print(s + "</font>");
        		}
        		else {
        			print ("no SafeJ information. </font>");
        		}
        		p();
        	}
        	else {
        		if (s != null && !s.equals("")) {
        			print ("<font color=\"green\"><B>Joe-E Comment:</B> " + s + "</font>");
            		p();
        		}
        	}
    ...
    public void writeImplementedInterfacesInfo() {
        ...
        /**
         * Check for any honorary implementations and print them out
         * @TODO: this is really poorly done. use writeClassLinks
         * We have to generate ClassDocs, or Types somehow, but they're all interfaces
         * I couldn't find an implementation of any of those interfaces. 
         */
        List honoraryInterfaces = configuration.consumer.getHonoraries(classDoc.qualifiedName());
        if (classDoc.isClass() && honoraryInterfaces != null && honoraryInterfaces.size() > 0) {
        	dl();
        	dt();
        	print("<B>Honorary Interfaces:</B>");
        	writeHonoraryInterfaces(LinkInfoImpl.CONTEXT_HONORARY_IMPLEMENTATIONS,
        		honoraryInterfaces);
    ...
    private void writeHonoraryInterfaces(int context, List list) {
    	Object[] typeList = list.toArray();
    	//Sort the list to be printed.
    	String basepath = "";
        int count = (classDoc.qualifiedName()).split("\\.").length - 1;
        for (; count > 0; count--) {
        	basepath += "../";
        }
        basepath += "org/joe_e/";
    	...
    	for (int i = 0; i < list.size(); i++) {
    		...
    		tempIntName = ((String) typeList[i]).split("\\.")[((String) typeList[i]).split("\\.").length-1];
    		print("<a style=\"color: green; visited: green\" href=\"" + basepath + tempIntName + ".html\">" + (String) typeList[i] + "</a>");
    	}
    	if (list.size() > 0) {
    		print("</font>");
    	}
