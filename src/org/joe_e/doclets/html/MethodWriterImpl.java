...
public class MethodWriterImpl extends AbstractExecutableMemberWriter 
    ...
    public void writeComments(Type holder, MethodDoc method) {
        ...
        String className = method.containingClass().qualifiedName();
    	MemberInfo m = writer.configuration.consumer.getInfo(method);
    	if (m == null) {
            writer.dd();
            if (writer.configuration.consumer.isDisabled(className)) {
                writer.print("<b>Class is disabled.</b>");
            } else {
                String signature =  method.name() + method.flatSignature();
