/*
 * @(#)HtmlDocletWriter.java	1.178 04/05/25
 *
...
import org.joe_e.doclets.html.markup.*;
...
public class HtmlDocletWriter extends HtmlDocWriter {
    ...
    public void printHtmlHeader(String title, String[] metakeywords, 
        ...
        printHtmlHeader(title, metakeywords, includeScript, false);
    }
     ...
     * @param includeScript boolean true if printing windowtitle script.
     *             False for files that appear in the left-hand frames.
     * @param disabled      true if documenting a disabled class
     */
    public void printHtmlHeader(String title, String[] metakeywords, 
    		boolean includeScript, boolean disabled) {
    	...
    	println("");
    	if (disabled) {
    		print("<BODY STYLE=\"background-color: #C9C9C9\"");
    		if (includeScript) {
    			print(" onload=\"windowTitle();\"");
    		}
    		print (">");
    		System.out.println("DISABLED " + title);
    	}
    	else {
    ...
    public String getLink(LinkInfoImpl linkInfo) {
        ...
        if (linkInfo.classDoc != null) {
        	if (configuration.consumer.isDisabled(linkInfo.classDoc.qualifiedName())) {
               	//System.out.println(linkInfo.classDoc.qualifiedName());
        		if (link.substring(0, 2).equals("<A")) {
        			link = "<A style=\"color: #444444; visited: #333333\" " + link.substring(3);
        		}
        	}
        }
        return link;
        ...
        if(begin >= 0){
            ...
            while(begin >=0){
                ...
                if(begin == 0){
                    ...
                    configuration.getDocletSpecificMsg().warning(doc.position(), 
